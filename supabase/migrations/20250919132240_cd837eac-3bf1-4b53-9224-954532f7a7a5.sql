...
-- Add unique constraint for tracking_code (with proper syntax)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_quotes_tracking') THEN
    ALTER TABLE public.quotes ADD CONSTRAINT uq_quotes_tracking UNIQUE (tracking_code);
  END IF;
END$$;
